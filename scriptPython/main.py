...
for mois, transactions in transactions_2025.items():
    ...
    doughnut_chart.add_series({
        ...
        'data_labels': {
            'percentage': True,
            'font': {'name': 'Helvetica', 'size': 10, 'font_color': '#1A1A1A'},
        },
        'points': [{'fill': {'color': color}} for color in colors[:len(categories)]],  # Couleurs des seg
    })
    doughnut_chart.set_title({
        'name': f"Catégories - {mois.capitalize()}",
        'name_font': {'bold': True, 'size': 25, 'font_color': '#1A1A1A', 'name': 'Helvetica'},  # Police du titre
    })
...
sorties_chart.add_series({
    ...
    'data_labels': {
        'value': True,             
        'font': {'bold': True, 'font_color': '#71D561', 'name' : 'Helvetica'},    
    },
    'fill': {'color': '#71D561'},   
    'border': {'color': '#71D561'}  
})
sorties_chart.set_title({
    'name': "Sorties d'argent",
    'name_font': {'bold': True, 'size': 25, 'font_color': '#71D561', 'name': 'Helvetica'},  
})
sorties_chart.set_x_axis({
    'name': 'Mois',
    'name_font': {'bold': True, 'size': 12, 'font_color': "#71D561", 'name' : 'Helvetica'},
    'num_font': {'font_color': "#71D561", 'name' : 'Helvetica'}, 
    'major_gridlines': {
        'visible': True,
...
entrees_chart.add_series({
    ...
    'data_labels': {
        'value': True,           
        'font': {'bold': True, 'font_color': '#002A00', 'name': 'Helvetica'},  
    },
    'fill': {'color': '#002A00'}, 
    'border': {'color': '#002A00'} 
})
entrees_chart.set_title({
    'name': "Entrées d'argent",
    'name_font': {'bold': True, 'size': 25, 'font_color': '#002A00', 'name': 'Helvetica'}, 
})
entrees_chart.set_x_axis({
    'name': 'Mois',
    'name_font': {'bold': True, 'size': 12, 'font_color': "#002A00", 'name': 'Helvetica'},
    'num_font': {'font_color': "#002A00", 'name': 'Helvetica'}, 
    'major_gridlines': {
        'visible': True,
        'line': {'color': '#D3D3D3', 'width': 1, 'dash_type': 'dash'},  
    },
...
balance_chart.set_title({
    'name': "Balance Mensuelle",
    'name_font': {'bold': True, 'size': 25, 'font_color': '#1A1A1A', 'name': 'Helvetica'}, 
})
balance_chart.set_x_axis({
    'name': 'Mois',
    'name_font': {'bold': True, 'size': 12, 'font_color': "#1A1A1A", 'name': 'Helvetica'},
    'num_font': {'font_color': "#1A1A1A", 'name': 'Helvetica'}, 
    'major_gridlines': {
        'visible': False, 
    },
